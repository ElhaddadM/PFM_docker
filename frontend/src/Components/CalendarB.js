...
function CalendarB() {
    ...
    const AllMois = []
     ...
     const fetchhada = () =>{
         axios.get(`http://127.0.0.1:8000/api/vacance/Reinscription`)
         .then((response) => {
    ...
    const selectedDate = (newValue, datee) => {
        ...
        axios.get(`http://127.0.0.1:8000/api/rdv/${datee}`)
            ...
            console.log('Hnaaa 1=> ', timeDispo.length );
            ...
            //     const RopoVa = {"Jour": datee.split('-')[2],"Mois": datee.split('-')[1],"Annee": datee.split('-')[0],"Service":"Reinscription"}
